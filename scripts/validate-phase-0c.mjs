...
const testRelease=/APP_VERSION='0\.7\.(?:2|3|4|5|6|7)-test'/.test(version)&&/APP_LABEL='V0\.7\.(?:2 DEMAND \+ OPPORTUNITY|3 READY \+ ENRICHMENT|4 CLIENT \+ BROKER TWIN|5 REVENUE OPERATIONS|6 OWNER \+ CAPTACION|7 REVENUE OS COMPLETE) TEST'/.test(version);
const productionRelease=/APP_VERSION='0\.7\.7'/.test(version)&&/APP_LABEL='V0\.7\.7 PRODUCTION'/.test(version);
if(!testRelease&&!productionRelease)throw new Error('Versión 0C+ incorrecta.');
