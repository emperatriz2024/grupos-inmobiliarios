...
const phase0b=await readFile(new URL('../db/migrations/002_phase_0b_identity_media_own.sql',import.meta.url),'utf8');
...
if(!/^BEGIN;[\s\S]*COMMIT;\s*$/i.test(phase0b))throw new Error('Migración 002 no es transaccional.');
console.log(`Core schema válido: ${required.length} tablas 0A + migración 0B.`);
