...
import {deploymentOrigin,sameDeploymentOrigin,workerToken} from '../../ingestion/worker-security.js';
...
const buildSha=()=>process.env.RADAR_BUILD_SHA||process.env.COMMIT_REF||process.env.BRANCH||'local';
export async function dispatchJob(jobId,{env=process.env,fetchImpl=fetch}={}){const token=workerToken(env);if(!token)throw new Error('worker_token_not_configured');const result=await fetchImpl(`${deploymentOrigin(env)}/.netlify/functions/ingestion-worker-background`,{method:'POST',headers:{'content-type':'application/json','x-radar-worker-token':token},body:JSON.stringify({job_id:jobId})});if(!result.ok)throw new Error('worker_dispatch_failed');}
export async function handler(event){
  ...
  try{
    ...
    if(body.action==='retry'){const job=await retryJob(body.job_id,{store});await dispatchJob(job.id);return response(202,{accepted:true,job_id:job.id});}
    const entries=await createServerDropbox().listPending(),created=await createBatch(entries,{store,buildSha:buildSha()});const dispatchErrors=[];
    for(const job of created.jobs)try{await dispatchJob(job.id);}catch(error){dispatchErrors.push(job.id);}
