...
export class DurableOutbox{
  ...
  async load(){if(this.loaded)return;await mkdir(path.dirname(this.file),{recursive:true});try{const rows=JSON.parse(await readFile(this.file,'utf8'));for(const row of rows)this.items.set(row.messageId,row);}catch(error){if(error.code!=='ENOENT')throw error;}this.loaded=true;}
  async persist(){const temp=`${this.file}.next`;await writeFile(temp,JSON.stringify([...this.items.values()]),{encoding:'utf8',mode:0o600});await rename(temp,this.file);}
  ...
  async retry(ids=[],attempt=1,now=Date.now()){await this.load();const delay=Math.min(300_000,1000*2**Math.min(8,attempt));for(const id of ids){const row=this.items.get(id);if(row)this.items.set(id,{...row,attempts:attempt,nextAttemptAt:now+delay});}await this.persist();return delay;}
...
export class MemoryOutbox{
  ...
  async retry(ids,attempt=1,now=Date.now()){const delay=Math.min(300_000,1000*2**Math.min(8,attempt));ids.forEach(id=>{const x=this.items.get(id);if(x)this.items.set(id,{...x,attempts:attempt,nextAttemptAt:now+delay});});return delay;}
