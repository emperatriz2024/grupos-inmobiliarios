...
export class SecondaryBridge{
  constructor({client,outbox,uploader,groupState=null,mediaCollector=null,intelligenceCollector=null,logger=()=>{},flushIntervalMs=15_000,scheduleFn=setTimeout}){Object.assign(this,{client,outbox,uploader,groupState,mediaCollector,intelligenceCollector,logger,flushIntervalMs,scheduleFn});this.state=BRIDGE_STATES.DISCONNECTED;this.timer=null;this.reconnectAttempt=0;this.stats={startedAt:new Date().toISOString(),groupsSeen:new Set(),messagesReceived:0,messagesQueued:0,messagesUploaded:0,duplicatesSkipped:0,lastMessageAt:null,lastSuccessfulUploadAt:null,backfillAttempts:0,backfillErrors:0};}
  setState(state,detail={}){this.state=state;this.logger(state,detail);}
  async accept(message){let event=await normalizeWhatsAppMessage(message);if(!event)return {ignored:true};this.stats.groupsSeen.add(event.groupId);this.stats.messagesReceived++;this.stats.lastMessageAt=event.receivedAt;this.logger('GROUP_MESSAGE_RECEIVED',{groupId:event.groupId,messageId:event.messageId});if(await this.groupState?.has(event.groupId,event.messageId)){this.stats.duplicatesSkipped++;this.logger('DUPLICATE_SKIPPED',{messageId:event.messageId,scope:'group_cursor'});return {duplicate:true};}if(event.hasMedia&&this.mediaCollector){const media=await this.mediaCollector(message);if(media)event={...event,media};}if(this.intelligenceCollector){const intelligence=await this.intelligenceCollector(event);if(intelligence)event={...event,intelligence};}const result=await this.outbox.enqueue(event);if(result.duplicate){this.stats.duplicatesSkipped++;this.logger('DUPLICATE_SKIPPED',{messageId:event.messageId});return result;}await this.groupState?.observe(event);this.stats.messagesQueued++;this.logger('QUEUED',{messageId:event.messageId});return result;}
