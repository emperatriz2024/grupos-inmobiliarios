...
const result=document.querySelector('#result'),STATE='phase-0c-e2e-state',RUN='phase-0c-e2e-run',DB='grupos-inmobiliarios',VERSION=13;
...
async function seed(){
  await getClients();const db=await open();assert(db.version===13,'IndexedDB no abrió V13');for(const name of ['clients','demands','demand_sources','match_runs','match_candidates','opportunities','opportunity_scores','opportunity_events'])assert(db.objectStoreNames.contains(name),`Falta ${name}`);
