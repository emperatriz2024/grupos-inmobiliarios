...
const result=document.querySelector('#result'),STATE='phase-0c-e2e-state',DB='grupos-inmobiliarios',VERSION=9;
const ids={buyer:'TEST-0C-BUYER',p1:'TEST-0C-P1',p2:'TEST-0C-P2',p3:'TEST-0C-P3',p4:'TEST-0C-P4'};
const requestAt=Date.now(),request=parseDemandRequest({text:'Solicito Casa San Diego hasta $60.000',messageId:'TEST-0C-REQUEST',author_id:'TEST-REQUESTER',group_id:'TEST-GROUP',received_at:new Date(requestAt).toISOString(),source_channel:'secondary_number'},{origin:'MARKET'});
...
async function seed(){
  ...
  tx.objectStore('buyers').put({id:'TEST-0C-PAUSED',name:'Cliente pausado',status:'paused',operation:'Venta',property_types:['Casa'],max_price:60000,created_at:now,updated_at:now});
...
async function run(){
  ...
  if(phase==='transitions'){
    const clients=await getClients(),demands=await getDemands(),opportunities=await getOpportunities();assert(clients.some(x=>x.legacy_buyer_id===ids.buyer),'Mirror client no persistió');assert(demands.some(x=>x.origin==='CLIENT')&&demands.some(x=>x.origin==='MARKET'),'Demandas CLIENT/MARKET no persistieron');assert(demands.find(x=>x.legacy_buyer_id==='TEST-0C-PAUSED')?.status==='PAUSED','Buyer paused no migró PAUSED');
    ...
    const repost=parseDemandRequest({text:'Solicito Casa San Diego hasta $60.000',messageId:'TEST-0C-REPOST',author_id:'TEST-REQUESTER',group_id:'TEST-GROUP',received_at:new Date(requestAt+5*86400000).toISOString(),source_channel:'secondary_number'},{origin:'MARKET'});await saveDemandRecords([repost]);assert((await getDemandSources(request.id)).length===2,'Repost real no preservó 2 sources');
