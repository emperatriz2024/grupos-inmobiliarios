...
async function seed(){
  await getClients();const db=await open();assert(db.version===9,'IndexedDB no abrió V9');for(const name of ['clients','demands','demand_sources','match_runs','match_candidates','opportunities','opportunity_scores','opportunity_events'])assert(db.objectStoreNames.contains(name),`Falta ${name}`);
