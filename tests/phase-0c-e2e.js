import {mirrorLegacyBuyersToDemands,saveDemandRecords,runDemandOpportunityMatching,getClients,getDemands,getDemandSources,getOpportunities,getOpportunityEvents,setMasterOwnership} from '../db.js';
import {parseDemandRequest} from '../core/radar/demand-engine.js';
import {APP_LABEL} from '../version.js';

const result=document.querySelector('#result'),STATE='phase-0c-e2e-state',RUN='phase-0c-e2e-run',DB='grupos-inmobiliarios',VERSION=10;
...
async function seed(){
  await getClients();const db=await open();assert(db.version===10,'IndexedDB no abrió V10');for(const name of ['clients','demands','demand_sources','match_runs','match_candidates','opportunities','opportunity_scores','opportunity_events'])assert(db.objectStoreNames.contains(name),`Falta ${name}`);
  ...
  await done(tx);db.close();await mirrorLegacyBuyersToDemands();await saveDemandRecords([request]);await runDemandOpportunityMatching({trigger_type:'FULL_RECALC'});
}
async function run(){
  ...
  if(phase==='transitions'){
    const clients=await getClients(),demands=await getDemands(),opportunities=await getOpportunities();assert(clients.some(x=>x.legacy_buyer_id===ids.buyer),'Mirror client no persistió');assert(demands.some(x=>x.origin==='CLIENT')&&demands.some(x=>x.origin==='MARKET'),'Demandas CLIENT/MARKET no persistieron');assert(demands.find(x=>x.legacy_buyer_id===ids.paused)?.status==='PAUSED','Buyer paused no migró PAUSED');
    const active=opportunities.filter(x=>x.status==='ACTIVE'),clientDemand=demands.find(x=>x.legacy_buyer_id===ids.buyer);
    const clientFixture=active.filter(x=>x.demand_id===clientDemand.id&&[ids.p1,ids.p2,ids.p3,ids.p4].includes(x.property_id)),marketFixture=active.filter(x=>x.demand_id===request.id&&[ids.p1,ids.p2,ids.p3,ids.p4].includes(x.property_id));
    assert(clientFixture.length===2&&clientFixture.some(x=>x.property_id===ids.p1)&&clientFixture.some(x=>x.property_id===ids.p2),'CLIENT no obtuvo P1+P2');assert(marketFixture.length===1&&marketFixture[0].property_id===ids.p1,'MARKET no quedó limitado a P1 OWN');
    const repost=parseDemandRequest({text:'Solicito Casa San Diego hasta $60.000',messageId:`TEST-0C-REPOST-${runKey}`,author_id:`TEST-REQUESTER-${runKey}`,group_id:`TEST-GROUP-${runKey}`,received_at:new Date(requestAt+5*86400000).toISOString(),source_channel:'secondary_number'},{origin:'MARKET'});await saveDemandRecords([repost]);assert((await getDemandSources(request.id)).length===2,'Repost real no preservó 2 sources');
    ...
    assert((await getOpportunities()).filter(x=>x.status==='ACTIVE'&&x.demand_id===request.id&&[ids.p1,ids.p2].includes(x.property_id)).length===2,'MARKET→OWN no abrió P2');
