...
test('botón ejecuta una búsqueda; segunda invalida primera y solo publica la última',async()=>{
 ...
 const ctx=vm.createContext({$,performance,setTimeout,getFilters:()=>({}),renderResults:()=>renders++,rememberSearchPosition(){},SearchWorkerController:class{search(f,mode,generation){return new Promise(resolve=>pending.push({resolve,generation}));}propertiesForIds(ids){return Promise.resolve(ids);}} });
 vm.runInContext('const globalThis={};let allProperties=[],currentResults=[],visibleCount=30;',ctx);
 ...
 const second=$('#searchBtn').onclick();assert.equal(pending.length,2);assert.ok(pending[1].generation>pending[0].generation);
