...
export async function setClientPropertyState(input,now=Date.now()){
  ...
  const changed=!previous||previous.status!==row.status||previous.reason!==row.reason||previous.evidence_at!==row.evidence_at;if(!changed){db.close();return previous;}
  await new Promise((resolve,reject)=>{const tx=db.transaction([CLIENT_PROPERTY_STATE_STORE,TWIN_EVENT_STORE],'readwrite');tx.objectStore(CLIENT_PROPERTY_STATE_STORE).put(row);tx.objectStore(TWIN_EVENT_STORE).put({id:`twin_event_${id}_${row.status}_${radarHash(`${at}|${row.reason||''}|${row.evidence_at||''}`)}`,buyer_id:row.buyer_id,property_id:row.property_id,event_type:`PROPERTY_${row.status}`,payload_json:{reason:row.reason},occurred_at:at});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);});db.close();return row;
}
...
// ---------- Property Twin + revenue pipeline (Phase 0F) --------------------
export async function rebuildPropertyTwins(now=Date.now()){
  const [properties,sources,states,matches,previous]=await Promise.all([getMasterProperties(),getAllSourcePosts(),getClientPropertyStates(),getAllMatches(),getPropertyTwins()]),prev=new Map(previous.map(x=>[x.property_id,x])),rows=properties.map(property=>buildPropertyTwin({property,sources:sources.filter(x=>x.master_id===property.id),clientStates:states.filter(x=>x.property_id===property.id),matches:matches.filter(x=>x.master_id===property.id),previous:prev.get(property.id),now}));const db=await openDB();await new Promise((resolve,reject)=>{const tx=db.transaction(PROPERTY_TWIN_STORE,'readwrite'),s=tx.objectStore(PROPERTY_TWIN_STORE);rows.forEach(x=>s.put(x));tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close();return rows;
}
export async function getPropertyTwins(){const db=await openDB(),rows=await reqP(db.transaction(PROPERTY_TWIN_STORE,'readonly').objectStore(PROPERTY_TWIN_STORE).getAll());db.close();return rows;}
export async function savePropertyTwinAction(propertyId,{next_action,next_action_at}){const db=await openDB(),s=db.transaction(PROPERTY_TWIN_STORE,'readonly').objectStore(PROPERTY_TWIN_STORE),old=await reqP(s.index('property_id').get(propertyId));if(!old){db.close();throw new Error('Property Twin no encontrado.')}const row={...old,next_action,next_action_at,updated_at:new Date().toISOString()};await new Promise((resolve,reject)=>{const tx=db.transaction(PROPERTY_TWIN_STORE,'readwrite');tx.objectStore(PROPERTY_TWIN_STORE).put(row);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close();return row;}
export async function getPipelines(){const db=await openDB(),rows=await reqP(db.transaction(PIPELINE_STORE,'readonly').objectStore(PIPELINE_STORE).getAll());db.close();return rows;}
export async function savePipeline(input,now=Date.now()){const id=`pipeline_${input.buyer_id}_${input.property_id||'general'}`,db=await openDB(),old=await reqP(db.transaction(PIPELINE_STORE,'readonly').objectStore(PIPELINE_STORE).get(id)),row=evolvePipeline(old,input,now),at=new Date(now).toISOString(),shape=x=>JSON.stringify({stage:x?.stage,owner:x?.owner,next_action:x?.next_action,next_action_at:x?.next_action_at,loss_reason:x?.loss_reason,opportunity_id:x?.opportunity_id}),fingerprint=shape(row),changed=!old||shape(old)!==fingerprint;if(!changed){db.close();return old;}await new Promise((resolve,reject)=>{const tx=db.transaction([PIPELINE_STORE,PIPELINE_EVENT_STORE],'readwrite');tx.objectStore(PIPELINE_STORE).put(row);tx.objectStore(PIPELINE_EVENT_STORE).put({id:`pipeline_event_${id}_${row.stage}_${radarHash(`${at}|${fingerprint}`)}`,pipeline_id:id,buyer_id:row.buyer_id,property_id:row.property_id,event_type:`STAGE_${row.stage}`,payload_json:{owner:row.owner,loss_reason:row.loss_reason},occurred_at:at});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)});db.close();return row;}
...
export async function saveOwnerTwin(input,now=Date.now()){const db=await openDB(),s=db.transaction(OWNER_TWIN_STORE,'readonly').objectStore(OWNER_TWIN_STORE),probe=buildOwnerTwin(input,null,now),old=await reqP(s.index('dedupe_key').get(probe.dedupe_key)),row=old?mergeOwners(old,probe,now):probe,at=new Date(now).toISOString(),shape=x=>JSON.stringify({confirmed:x?.confirmed,pending:x?.pending,unavailable:x?.unavailable,property_ids:x?.property_ids,documents:x?.documents,contact:x?.contact,last_interaction_at:x?.last_interaction_at,next_action:x?.next_action,next_action_at:x?.next_action_at}),fingerprint=shape(row),changed=!old||shape(old)!==fingerprint;if(!changed){db.close();return old;}await new Promise((resolve,reject)=>{const tx=db.transaction([OWNER_TWIN_STORE,OWNER_EVENT_STORE],'readwrite');tx.objectStore(OWNER_TWIN_STORE).put(row);tx.objectStore(OWNER_EVENT_STORE).put({id:`owner_event_${row.id}_${radarHash(`${at}|${fingerprint}`)}`,owner_id:row.id,event_type:old?'OWNER_UPDATED':'OWNER_CREATED',payload_json:{property_ids:row.property_ids},occurred_at:at});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)});db.close();return row;}
...
export async function saveVisit(input,now=Date.now()){const probe=evolveVisit(null,input,now),db=await openDB(),old=await reqP(db.transaction(VISIT_STORE,'readonly').objectStore(VISIT_STORE).get(probe.id)),row=evolveVisit(old,input,now),fingerprint=JSON.stringify({...row,updated_at:null}),changed=!old||JSON.stringify({...old,updated_at:null})!==fingerprint;await new Promise((resolve,reject)=>{const tx=db.transaction(changed?[VISIT_STORE,VISIT_EVENT_STORE]:[VISIT_STORE],'readwrite');tx.objectStore(VISIT_STORE).put(changed?row:old);if(changed)tx.objectStore(VISIT_EVENT_STORE).put({id:`visit_event_${row.id}_${row.stage}_${radarHash(`${row.updated_at}|${fingerprint}`)}`,visit_id:row.id,event_type:`STAGE_${row.stage}`,payload_json:{outcome:row.outcome},occurred_at:row.updated_at});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)});db.close();return changed?row:old;}
...
export async function saveDeal(input,now=Date.now()){const id=`deal_${input.buyer_id}_${input.property_id}`,db=await openDB(),old=await reqP(db.transaction(DEAL_STORE,'readonly').objectStore(DEAL_STORE).get(id)),row=evolveDeal(old,input,now),fingerprint=JSON.stringify({...row,updated_at:null}),changed=!old||JSON.stringify({...old,updated_at:null})!==fingerprint;await new Promise((resolve,reject)=>{const tx=db.transaction(changed?[DEAL_STORE,DEAL_EVENT_STORE]:[DEAL_STORE],'readwrite');tx.objectStore(DEAL_STORE).put(changed?row:old);if(changed)tx.objectStore(DEAL_EVENT_STORE).put({id:`deal_event_${row.id}_${row.stage}_${radarHash(`${row.updated_at}|${fingerprint}`)}`,deal_id:row.id,event_type:`STAGE_${row.stage}`,payload_json:{offer_amount:row.offer_amount,counter_amount:row.counter_amount,loss_reason:row.loss_reason},occurred_at:row.updated_at});tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)});db.close();return changed?row:old;}
