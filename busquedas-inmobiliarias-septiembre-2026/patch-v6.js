...
function render6(a,wantOp){
 ...
 root.innerHTML=a.slice(0,300).map(p=>{const D=d6(p),L=D.loc,T=D.type,S=D.stats,C=D.captor,wa=wa6(p,C);
  ...
  const waBtn=wa?'<button class="waIcon" data-wa6="'+e6(wa)+'">WhatsApp</button>':'<span class="hint" style="margin-left:auto">Sin teléfono</span>';
  const specItems=[[S.m2,'m²'],[S.h,'hab'],[S.b,'baños'],[S.e,'puestos']].filter(([v])=>v!=null&&v!=='').map(([v,lbl])=>'<div class="spec"><b>'+e6(v)+'</b><span>'+lbl+'</span></div>').join('');
  const rawTxt=raw6(p),half=Math.max(200,Math.ceil(rawTxt.length/2)),preview=rawTxt.length>half?rawTxt.slice(0,half).trim():rawTxt,restTxt=rawTxt.length>half?rawTxt.slice(half):'';
  const original=restTxt
    ? '<pre style="white-space:pre-wrap;font-size:12px;line-height:1.5;margin:0;opacity:.82;font-family:inherit">'+e6(preview)+'<span class="more6" style="display:none">'+e6(restTxt)+'</span></pre><button class="showMore6" style="width:auto;padding:4px 0;margin-top:3px;border:0;background:none;font-size:12px;font-weight:700;color:#4e20d3">Ver mensaje completo</button>'
    : '<pre style="white-space:pre-wrap;font-size:12px;line-height:1.5;margin:0;opacity:.82;font-family:inherit">'+e6(rawTxt)+'</pre>';
 return'<article class="card"><div class="head"><div><div><span class="pill">'+e6(opLabel6(p,wantOp))+'</span>'+(T?'<span class="pill">'+e6(T)+'</span>':'')+stageBadges+'</div><div class="loc">'+e6(loc)+'</div><div class="sender"><b>Captador:</b> '+e6(C.name)+'</div></div><div><div class="price">'+priceLabel6(p,wantOp)+'</div><div class="age">'+e6(ageLabel6(p))+'</div></div></div><div class="specs">'+specItems+waBtn+'</div><div class="features">'+f+'</div>'+hist+'<div style="border-top:1px solid var(--line);padding-top:8px;margin-top:8px">'+original+'</div></article>'}).join('');
 root.querySelectorAll('.showMore6').forEach(b=>b.onclick=()=>{const m=b.previousElementSibling?.querySelector('.more6');if(m){m.style.display='inline';b.remove()}});
...
window.RI6={type6,loc6,op6,captor6,d6,request6,features6,project6,raw6,dedupe6,priceFor6,priceLabel6,opLabel6,ageLabel6,wa6,cash6,typesAll6,municipiosAll6,ts6};
})();