...
function loc6(p){const x=n6(raw6(p));for(const[mun,zs]of Object.entries(LOC6))for(const[z,aa]of Object.entries(zs))if(aa.some(a=>x.includes(n6(a))))return{municipio:mun,zona:z};if(/\bsan\s+diego\b/.test(x))return{municipio:'San Diego',zona:p?.zona||null};if(/\bnaguanagua\b/.test(x))return{municipio:'Naguanagua',zona:p?.zona||null};if(/\blos\s+guayos\b/.test(x))return{municipio:'Los Guayos',zona:p?.zona||null};if(/\bvalencia\b/.test(x))return{municipio:'Valencia',zona:p?.zona||null};return{municipio:p?.municipio||p?.municipality||null,zona:p?.zona||p?.zone||null}}
function request6(p){const x=n6(raw6(p)),h=x.slice(0,1000);if(/^\W*(?:solicitud|solicito|busco|se busca|cliente busca|requiero|necesito)\b/.test(h))return true;const req=/\b(?:solicitud|solicito|solicita|se solicita|busco|se busca|cliente busca|requiero|requiere|necesito|buscando|presupuesto hasta|para la compra)\b/.test(h),listing=/\b(?:se vende|vendo|vende|en venta|se alquila|alquilo|en alquiler|disponible|nueva captacion|ofrece en venta|ofrece en alquiler)\b/.test(h);return req&&!listing}
...
function phones6(s){const out=[],re=/(?:\+?58[\s().\-]*)?0?(?:412|414|416|424|426)(?:[\s().\-]*\d){7}/g;let m;while((m=re.exec(String(s||'')))){let d=m[0].replace(/\D/g,'');if(d.startsWith('58'))d=d.slice(2);if(d.startsWith('0'))d=d.slice(1);if(d.length===10&&/^(412|414|416|424|426)/.test(d))out.push('+58'+d)}return[...new Set(out)]}
function cleanPerson6(s){let x=String(s||'').replace(/(?:\+?58[\s().\-]*)?0?(?:412|414|416|424|426)(?:[\s().\-]*\d){7}/g,' ').replace(/[*_~•📲📞☎️👉➡️✅🔹🔸]/g,' ');x=x.replace(/\b(?:tlf|tel[eé]fono|telefono|celular|whatsapp|contacto|contacta|asesor(?:a)?|inmobiliari[oa]|licda?|lic|econ|realtor)\b[:.\s-]*/gi,' ');return x.replace(/[^\p{L}\s.'-]/gu,' ').replace(/\s+/g,' ').trim()}
...
function tokens6(p){let x=n6(raw6(p));x=x.replace(/https?:\/\/\S+/g,' ').replace(/(?:\+?58[\s().\-]*)?0?(?:412|414|416|424|426)(?:[\s().\-]*\d){7}/g,' ').replace(/\b\d{1,2}[\/.\-]\d{1,2}[\/.\-]\d{2,4}\b/g,' ').replace(/\b\d+(?:[.,]\d+)?\b/g,' ').replace(/[^a-zñ\s]/g,' ');return new Set(x.split(/\s+/).filter(w=>w.length>=4&&!STOP6.has(w)))}
