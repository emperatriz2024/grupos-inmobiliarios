...
function project6(p){const x=n6(raw6(p));const r=/(?:res(?:idencias?|\.)?|conjunto(?:\s+residencial)?|urb(?:anizacion|\.)?|edificio|villa(?:s)?|residencial)\s+([a-z0-9ñ ]{3,45})/g;let m,best='';while((m=r.exec(x))){let z=m[1].replace(/\b(?:ubicado|ubicada|en|san diego|valencia|edo|carabobo|cuenta|consta|con|venta|alquiler|moderno|moderna|amplio|amplia|hermoso|hermosa|bonito|bonita|lujoso|lujosa|comodo|comoda|espectacular)\b.*$/,'').replace(/\d.*$/,'').trim();if(z.length>best.length)best=z}return best}
