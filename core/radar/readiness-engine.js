...
import {daysSince} from '../../freshness-utils.js';
...
const CLOSED=new Set(['SOLD','RENTED','ARCHIVED','EXPIRED','INACTIVE','UNAVAILABLE','sold','rented','archived','expired','inactive','unavailable']);
const REQUIRED_FACTS=['operation','property_type','price_usd'];
...
function priceConflict(property,evidence){
  if(property.price_conflict===true||String(property.price_audit_status||'').toUpperCase()==='CONFLICT')return true;
  const values=factValues(property,'price_usd',evidence).map(Number).filter(Number.isFinite);
  return new Set(values).size>1;
}

function availability(property,now){
  if(CLOSED.has(property.status))return {blocked:true,verify:false,reason:'PROPERTY_NOT_ACTIVE'};
  const verifiedAge=daysSince(property.last_verified_at,now),seenAge=daysSince(property.last_seen_at||property.updated_at||property.first_seen_at,now);
  if(property.availability_status==='verified'&&verifiedAge<=7)return {blocked:false,verify:false};
  if(property.availability_status==='unavailable')return {blocked:true,verify:false,reason:'PROPERTY_UNAVAILABLE'};
  if(verifiedAge>7&&seenAge>7)return {blocked:false,verify:true,reason:'AVAILABILITY_NOT_RECENT'};
  return {blocked:false,verify:false};
}
...
export function assessOpportunity({opportunity,property,evidence=[],mediaAssets=[],propertyMedia=[],identityLinks=[],reviewQueue=[],now=Date.now()}={}){
  if(!opportunity||!property)throw new Error('opportunity_and_property_required');
  let status='READY',reasons=[],missing_fields=[],tasks=[];
  const available=availability(property,now);
  if(opportunity.status!=='ACTIVE'||available.blocked){status='BLOCKED';reasons.push(available.reason||'OPPORTUNITY_NOT_ACTIVE');}
  else if(priceConflict(property,evidence)){status='PRICE_CONFLICT';reasons.push('ACTIVE_PRICE_EVIDENCE_CONFLICTS');tasks.push(task(opportunity,'VERIFY_PRICE',now));}
    ...
    missing_fields=REQUIRED_FACTS.filter(field=>factValues(property,field,evidence).length===0);
  ...
  return {assessment:{id:`readiness_${stableId(opportunity.id)}`,workspace_id:opportunity.workspace_id||'local',opportunity_id:opportunity.id,property_id:opportunity.property_id,status,reasons,gaps:missing_fields,readiness_score:Math.max(0,100-penalties[status]),assessed_at:iso(now),updated_at:iso(now)},tasks};
}
...
export function buildClientPackage({opportunity,property,assessment,mediaAssets=[],propertyMedia=[],now=Date.now()}={}){
  ...
  return {package:{id:`package_${stableId(opportunity.id)}`,workspace_id:opportunity.workspace_id||'local',opportunity_id:opportunity.id,property_id:property.id,status:'READY',payload_json:payload,created_at:iso(now),updated_at:iso(now)},media:media.map((row,index)=>({id:`package_media_${stableId(opportunity.id)}_${stableId(row.id)}`,package_id:`package_${stableId(opportunity.id)}`,media_asset_id:row.id,sort_order:row.sort_order??index,is_primary:row.is_primary,created_at:iso(now)}))};
}

export function reconcileReadiness({opportunities=[],properties=[],evidence=[],mediaAssets=[],propertyMedia=[],identityLinks=[],reviewQueue=[],scope={},now=Date.now()}={}){
