...
const intelligenceText=event=>{const ai=event?.intelligence;if(!ai||typeof ai!=='object')return '';return [ai.summary,ai.property_type,ai.operation,ai.zone,ai.municipality,ai.residence,ai.price_usd&&`$${ai.price_usd}`,ai.area_m2&&`${ai.area_m2} m2`,ai.bedrooms&&`${ai.bedrooms} habitaciones`,ai.bathrooms&&`${ai.bathrooms} baños`,ai.parking&&`${ai.parking} puestos`,ai.contact_phone,(ai.amenities||[]).join(' ')].filter(Boolean).join(' · ');};
...
export function groupConsecutiveEvents(events=[],windowMs=120_000){
  ...
  for(const event of sorted){const previous=groups.at(-1),body=[event.text,event.caption,intelligenceText(event)].filter(Boolean).join('\n').trim();if(!body)continue;
    const canJoin=previous&&previous.groupId===event.groupId&&previous.authorId===event.authorId&&Date.parse(event.timestamp)-Date.parse(previous.lastAt)<=windowMs&&!isDemandRequest(previous.text)&&!isDemandRequest(body)&&(looksLikeRealEstate(previous.text)||LISTING_LEAD.test(previous.text))&&CONTINUATION_SIGNAL.test(body);
...
export function processSecondaryEvents(rawEvents=[],{locationCatalog=null,resolveTerritory=null}={}){
  ...
  const records=[],demands=[],nonProperty=[],attachments=valid.filter(event=>event.hasMedia).map(event=>({id:`att_secondary_${event.messageId}`,source_message_id:null,external_message_id:event.messageId,external_media_id:event.media?.sha256||event.messageId,provenance_status:'UNRESOLVED',media_type:/image/i.test(event.mediaType)?'IMAGE':/video/i.test(event.mediaType)?'VIDEO':/audio|ptt/i.test(event.mediaType)?'AUDIO':/document/i.test(event.mediaType)?'DOCUMENT':'UNKNOWN',mime_type:event.media?.mimeType||null,original_filename:event.media?.filename||null,size_bytes:event.media?.sizeBytes||null,width:null,height:null,duration_ms:null,sha256:event.media?.sha256||null,storage_locator:event.media?.storageLocator||null,thumbnail_locator:event.media?.thumbnailLocator||null,media_status:event.media?'STORED':'OBSERVED',received_at:event.receivedAt,ingested_at:new Date().toISOString(),metadata_json:{source_type:event.sourceType,source_channel:event.sourceChannel,group_id:event.groupId,group_name:event.groupName,message_type:event.messageType},created_at:new Date().toISOString()}));
  ...
  return {records,demands,attachments,validCount:valid.length,invalid,nonProperty,propertiesDetected:records.length,demandsDetected:demands.length,groupsDetected:new Set(valid.map(x=>x.groupId)).size};
}
